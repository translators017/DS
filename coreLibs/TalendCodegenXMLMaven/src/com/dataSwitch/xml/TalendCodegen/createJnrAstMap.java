...
public class createJnrAstMap 
{

	public static void fillJoinerAstMapTrans(TransRecExtDTO recordperPipe, Connector docspecs, Node compNode,
			SrcTgtInfo srctgtdb) {
		...
		String Exptranstype = "tMap";
		  ...
		  JoinerInfo jnr = (JoinerInfo) recordperPipe.transInfo;
		  String mstrcompName = Utils.getMstrCompForJnr(jnr,compName);
		  ...
		  for(int j = 0 ; j < jnr.masterCols.size(); j++)
		  {
			  if(j >= jnr.detailCols.size())
			  		continue;
				ColumnDTO jnrMstrName1 = jnr.masterCols.get(j);
				String jnrMstrName = jnrMstrName1.colStruct.name;
				ColumnDTO jnrDtlsName1 = jnr.detailCols.get(j);
...
//				String dtlColName = jnrDtlsName1.aliasName;
				...
				keyColsMap.put(jnrDtlsName1.aliasName, mstrColName);
		  }
		...
		/*  ArrayList<String> masterInputColList = new ArrayList<String>();
		  ...
		  for(int j = 0 ; j < recordperPipe.inRecsList.size() ; j++)
		  {
			  ...
			  if(!prevoutRec.equalsIgnoreCase(mstrcompName))
			  {
				  ...
				  if(jnr.joinType.equalsIgnoreCase("Inner Join") || jnr.joinType.equalsIgnoreCase("Inner"))
				  {
