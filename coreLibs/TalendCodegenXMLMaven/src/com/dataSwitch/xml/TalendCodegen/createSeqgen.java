...
public class createSeqgen {
	public static void fillSeqGenTrans(TransRecExtDTO recordperPipe,Connector docspecs,Node compNode,SrcTgtInfo srctgtdb)
	{
		 String Exptranstype = "tMap";
		  ...
		  SeqGenInfo seqGen = (SeqGenInfo) recordperPipe.transInfo;
...
//				 colm.setPrecision(precision);
			  ...
			  for(int i1 = 0 ; i1 < seqGen.colDTOList.size() ; i1++)
			  {
				  ...
				  String colName = colsName.substring(colsName.indexOf(".")+1, colsName.length());
				  if(SeqCols.transRule.transRuleName.equalsIgnoreCase("Special - Surrogate Key Generation"))
				  {
					  ...
					  mapTbl.setexpression("Numeric.sequence(\"s1\","+paramVal+",1)");
