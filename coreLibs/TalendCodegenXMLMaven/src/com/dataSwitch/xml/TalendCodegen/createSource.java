...
//					colDTO.get(0).colStruct.name;
	...
	public static void fillPropforDBSrc(TransRecExtDTO recordperPipe,Connector docspecs,Node compNode,SrcTgtInfo srctgtdb,String szoutputLinkedServiceFilePath, List<String> cntxtParamsList, LookupInfo lkp)
	{
			...
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - MySQL"))
			{
				tlndTranstype = "MySQL";
			}
		...
		String[] values = new String[]{compName,"(To use this component, you need first to add nzjdbc.jar in the Modules view)","false","null","read from docspecs",tlndTranstype,"read from docspecs","get from reclist","read from docspecs","read from docspecs","\""+srctgtdb.tableName+"\"","\"\"","","BUILT_IN",
			...
			if(values[i]!=""){
				if(values[i].equalsIgnoreCase("get from reclist")){
					if(names[i].equalsIgnoreCase("QUERY")){
						String query = "";
						if(lkp!=null) {
							for(int k=0; k<lkp.lkpSrcList.size(); k++) {
								String srcname = lkp.lkpSrcList.get(k).lookupElemList.get(0);
								srcname = srcname.substring(0,srcname.indexOf('.'));
								if(srcname.equalsIgnoreCase(compName)) {
									query = lkp.lkpSrcList.get(k).sqlOverride;
								}
							}
						}
						else {
							if(srctgtdb.overrideSQL.length() > 0)
								query = srctgtdb.overrideSQL;
							else
								query = constructQuery(srctgtdb.srcTgtColsList, srctgtdb.tableName);
						}
						query = query.replaceAll("MappingVariable", "context");
						elemParam.setValue("\""+query+"\"");
					}
					if(names[i].equalsIgnoreCase("DBNAME")){
					
						if(uNamePwd.get("schemaName")!=null){
							String dbName = uNamePwd.get("schemaName");
					...
					if(names[i].equalsIgnoreCase("HOST")){
						if(uNamePwd.get("host")!=null){
							host = uNamePwd.get("host");
							//host = Utils.getParamValWithContext(pwd, docspecs, cntxtParamsList);
						}
						elemParam.setValue("\""+host+"\"");
					}
					if(names[i].equalsIgnoreCase("PORT")){
						if(uNamePwd.get("port")!=null){
							port = uNamePwd.get("port");
							//port = Utils.getParamValWithContext(pwd, docspecs, cntxtParamsList);
						}
						elemParam.setValue("\""+port+"\"");
					}
				}
		...
		if(srctgtdb.fileType.equalsIgnoreCase("Relational - Azure SQL DB") || srctgtdb.fileType.equalsIgnoreCase("Relational - Azure SQL Data Warehouse")){
			TalendfileProcessType.Node.ElementParameter elemParam = new TalendfileProcessType.Node.ElementParameter();
			elemParam.setField("CLOSED_LIST");
			elemParam.setName("DRIVER");
			elemParam.setValue("MSSQL_PROP");
			compNode.getElementParameter().add(elemParam);
		}
		else if(srctgtdb.fileType.equalsIgnoreCase("Relational - MySQL"))
		{
			TalendfileProcessType.Node.ElementParameter elemParam = new TalendfileProcessType.Node.ElementParameter();
			elemParam.setField("CLOSED_LIST");
			elemParam.setName("DB_VERSION");
			elemParam.setValue("MYSQL_8");
			compNode.getElementParameter().add(elemParam);
		}
		
	}
	
	private static String constructQuery(ArrayList<ColumnDTO> srcTgtColsList, String tableName) 
	{
		String Query = "";
		Query += "SELECT";
	    for(int i = 0 ; i < srcTgtColsList.size(); i++)
	    {
	    	ColumnDTO colsDto = srcTgtColsList.get(i);
	    	String colName = colsDto.colStruct.name;
	    	
	    	if(i == srcTgtColsList.size()-1)
	    	{
	    		Query += "\n" + "`"+tableName+"`"+"."+"`"+colName+"`";
	    		Query += "\n" + "FROM "+"`"+tableName+"`";
	    	}
	...
	public static void fillOutColsforSrc(TalendfileProcessType pipeline, TransRecExtDTO recordperPipe,Connector docspecs,Node compNode,SrcTgtInfo srctgtdb,String szoutputLinkedServiceFilePath, DataMapping dataMappingObj, List<String> cntxtParamsList, ReportLogInfo reportObj, LookupInfo lkp)
	{
		String preSQL = "";
		TalendfileProcessType.Node preSQLNode = new TalendfileProcessType.Node();
		if(srctgtdb.preSQL!=null){
			preSQL = srctgtdb.preSQL;
			preSQL = preSQL.replaceAll("MappingVariable", "context");
		}
		
		String postSQL = "";
		TalendfileProcessType.Node postSQLNode = new TalendfileProcessType.Node();
		if(srctgtdb.postSQL!=null){
			postSQL = srctgtdb.postSQL;
			postSQL = postSQL.replaceAll("MappingVariable", "context");
		}
		
		if(!preSQL.equalsIgnoreCase(""))
			CreatePreSqlComp.createSrcForPreSQL(pipeline,recordperPipe,docspecs,preSQLNode,preSQL,srctgtdb,szoutputLinkedServiceFilePath,dataMappingObj,cntxtParamsList);
		if(!postSQL.equalsIgnoreCase(""))
			CreatePostSqlComp.createSrcForPostSQL(pipeline,recordperPipe,docspecs,postSQLNode,postSQL,srctgtdb,szoutputLinkedServiceFilePath,dataMappingObj,cntxtParamsList);
		
		
		reportObj.transTypeList.put(recordperPipe.compName, "Source Component");
		ArrayList<String> manualChangesList = new ArrayList<String>();
		if(srctgtdb.overrideSQL!=null && !srctgtdb.overrideSQL.equalsIgnoreCase("")){
			manualChangesList.add("Overrid SQL to be handled manually");
		}
		...
		if(srctgtdb.fileExtension != null)
		{
			...
			{
				tlndTranstype = "tFileInputExcel";
				DataBaseType = ToolTypeConstants.FLATFILE;
			}
		...
		else
		{
			if(srctgtdb.fileType.equalsIgnoreCase("Relational - Netezza"))
			{
				...
				DataBaseType = ToolTypeConstants.NETEZZA;
			}
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - Teradata"))
			{
				tlndTranstype = "tTeradataInput";
				DataBaseType = ToolTypeConstants.TERADATA;
			}
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - DB2 LUW"))
			{
				tlndTranstype = "tDB2Input";
				DataBaseType = ToolTypeConstants.DB2;
			}
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - Oracle"))
			{
				tlndTranstype = "tOracleInput";
				DataBaseType = ToolTypeConstants.ORACLE;
			}
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - ODBC"))
			{
				tlndTranstype = "tOracleInput";
			...
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - SQL Server"))
			{
				...
				DataBaseType = ToolTypeConstants.SQLSERVER;
			}
			...
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - Snowflake DW"))
			{
				...
				DataBaseType = ToolTypeConstants.SNOWFLAKE;
			}
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - Amazon Redshift"))
			{
				tlndTranstype = "tRedshiftInput";
				DataBaseType = ToolTypeConstants.REDSHIFT;
			}
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - MySQL"))
			{
				tlndTranstype = "tMysqlInput";
				DataBaseType = ToolTypeConstants.MYSQL;
			}
		}
		...
		if(srctgtdb.fileExtension != null)
		{
			...
			{
				compVersion = 0.102;
				createSource.fillPropforflatSrc(recordperPipe, docspecs, compNode, srctgtdb,szoutputLinkedServiceFilePath);
			}
			else
			{
				compVersion = 0.102;
				createSource.fillPropforflatTxtSrc(recordperPipe, docspecs, compNode, srctgtdb,szoutputLinkedServiceFilePath,cntxtParamsList);
			}
			/*else if(srctgtdb.fileExtension.equalsIgnoreCase(".txt"))
			{
				compVersion = 0.102;
				createSource.fillPropforflatTxtSrc(recordperPipe, docspecs, compNode, srctgtdb,szoutputLinkedServiceFilePath);
			}
			else if(srctgtdb.fileType.equalsIgnoreCase("Files - Fixed Width"))
			{
				compVersion = 0.102;
				createSource.fillPropforflatTxtSrc(recordperPipe, docspecs, compNode, srctgtdb,szoutputLinkedServiceFilePath);
			}*/
		...
		else
		{
			if(srctgtdb.fileType.equalsIgnoreCase("Relational - Netezza")||srctgtdb.fileType.equalsIgnoreCase("Relational - Teradata")||
					...
					srctgtdb.fileType.equalsIgnoreCase("Relational - ODBC") || srctgtdb.fileType.equalsIgnoreCase("Relational - Amazon Redshift") || srctgtdb.fileType.equalsIgnoreCase("Relational - SQL Server") || srctgtdb.fileType.equalsIgnoreCase("Relational - Azure SQL DB") 
					|| srctgtdb.fileType.equalsIgnoreCase("Relational - Azure SQL Data Warehouse") || srctgtdb.fileType.equalsIgnoreCase("Relational - MySQL")) 
			{
				compVersion = 0.102;
				createSource.fillPropforDBSrc(recordperPipe, docspecs, compNode, srctgtdb,szoutputLinkedServiceFilePath, cntxtParamsList,lkp);
			}
			else if(srctgtdb.fileType.equalsIgnoreCase("Relational - Snowflake DW"))
			{
				compVersion = 0.102;
				createSource.fillPropforDBSnowFlakeSrc(recordperPipe, docspecs, compNode, srctgtdb,szoutputLinkedServiceFilePath,cntxtParamsList,dataMappingObj,lkp);
			}
		}
		...
		TalendfileProcessType.Node.Metadata rjtmetadata = null;
		if(srctgtdb.fileExtension != null)
		{
			rjtmetadata = new TalendfileProcessType.Node.Metadata();
			rjtmetadata.setConnector("REJECT");
			rjtmetadata.setName("REJECT");
		}
		     ...
		     for( int j = 0 ; j < srctgtdb.srcTgtColsList.size() ; j++)
		     {
				...
				if(cols.colStruct.dateFormat != null)
				{
					if(cols.colStruct.dateFormat.length() > 0)
						tlndCol.setPattern("\""+cols.colStruct.dateFormat+"\"");
				}
				else
					tlndCol.setPattern("");
				...
				if(srctgtdb.fileType.equalsIgnoreCase("Relational - ODBC"))
				{
					String tempDataType = dtc.getTargetDataType(dataType,ToolTypeConstants.ODBC,DataBaseType);
					if(tempDataType.contains("NUMBER(")){
						tempDataType= "NUMBER";
					}
					dataType = dtc.getTargetDataType(tempDataType,DataBaseType,ToolTypeConstants.TALEND);
				}
				else{
				...
				if(srctgtdb.fileExtension == null)
				{
					String tgtDataType =  cols.colStruct.dataType;
					if(tgtDataType.equalsIgnoreCase("Number(p,s)"))
					{
						tgtDataType = "NUMBER";
					}
					tlndCol.setsourceType(tgtDataType.toUpperCase());
				}
				...
				if(srctgtdb.fileExtension != null)
				{
					rjtmetadata.getColumn().add(tlndCol);
				}
	}
		compNode.getMetadata().add(metadata);
		compNode.getMetadata().add(rjtmetadata);
	}
	...
	public static void fillPropforDBSnowFlakeSrc(TransRecExtDTO recordperPipe, Connector docspecs, Node compNode,
			SrcTgtInfo srctgtdb, String szoutputLinkedServiceFilePath, List<String> cntxtParamsList, DataMapping dataMappingObj, LookupInfo lkp) 
	{
		ArrayList<ElementParameter> elemParam = new ArrayList<ElementParameter>();
		elemParam.add(Utils.getPropDtlsWithShwforExp("TEXT","UNIQUE_NAME",recordperPipe.compName,"false"));
		...
		if(lkp!=null) {
			for(int k=0; k<lkp.lkpSrcList.size(); k++) {
				String srcname = lkp.lkpSrcList.get(k).lookupElemList.get(0);
				srcname = srcname.substring(0,srcname.indexOf('.'));
				if(srcname.equalsIgnoreCase(recordperPipe.compName)) {
					updatedQuery = lkp.lkpSrcList.get(k).sqlOverride;
				}
			}
		}
		else {
			updatedQuery = srctgtdb.overrideSQL;
		}
		updatedQuery = updatedQuery.replaceAll("MappingVariable", "context");
