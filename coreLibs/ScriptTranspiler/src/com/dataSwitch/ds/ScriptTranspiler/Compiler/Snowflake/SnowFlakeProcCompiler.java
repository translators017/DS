...
import com.dataSwitch.ds.Script.StatementObjts.dsWhile;
	...
	public class SnowFlakeProcCompiler 
	{
		public static void getProcValues(String fileType, LinkedList<iStatement> sqlQryList, iStatement istatDtls, int newDbId, int currentDbId, String outpath, String filename, String configJsonPath, Console console, LinkedHashMap<String, LinkedHashMap<String, String>> configHash)
		{
			...
			if (iStDet.statDetail instanceof dsProc)//Procedure
			{
				...
				String procArgVals = getArgsVal(iProc);
				...
				if (iProc.asBlock != null)
				{
					if (iProc.asBlock instanceof iBlockStatement)
					{
						...
						updatedVal += tmpTab + "var ds_v_sqlCommand;\n";
						updatedVal = processBlkSt (iProc.asBlock, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
					}
				}
				if (iProc.procBlockStatements != null)
				{
					if (iProc.procBlockStatements instanceof iBlockStatement)
					{
						...
						updatedVal = processBlkSt (iProc.procBlockStatements, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
					}
				}
				if (newDbId == ToolTypeConstants.SNOWFLAKE)
				{
					...
					for (int i = 0; i < throwsVal.size(); i++) 
					{
						tmp += throwsVal.get(i);
					}
					String tmpArr[] = {"'Failed: Code: '","'State: '","'Message: '","'Stack Trace:'"}; 
					tmp += "\tvar result = "+tmpArr[0]+" + err.code ;\n "+
							"\tresult += "+tmpArr[1]+" + err.state; \n"+
							"\tresult += "+tmpArr[2]+" + err.message;\n"+
							"\tresult += "+tmpArr[3]+" + err.stackTraceTxt; \n";
					tmp += "\treturn 'Failed : ' + result;\n";
					tmp += "}\n\n$$;";
					updatedVal += tmp;
				}
			}
...
//									dataType = dtc.getTargetDataType(tkn.get(0).data, currentDbId, newDbId);
		...
		private static String processBlkSt(iBlockStatement iBlockSt, int newDbId, int currentDbId, String updatedVal, ReportLogInfo rptLogInfo, LinkedHashMap<String, LinkedHashMap<String, String>> configHash, int qryCnt, int tabCnt,
				ArrayList<String> throwsVal, ArrayList<Integer> statementCnt, LinkedHashMap<ArrayList<String>, Integer> manualChangesList,LinkedList<SQLValue> sqLValList, ArrayList<String> totalAttributeCount, ArrayList<String> totalTransCount, LinkedList<iStatement> sqlQryList) 
		{
			String tmpTab = "";
			if (tabCnt > 0)
				tmpTab = getTabSpace (tabCnt, tmpTab);
			...
			if (iBlockSt != null)
			{
				if (iBlockSt.iBlockStatements.size() > 0)
				{
					...
					for(Entry<Float, iStatement> entrySet : iBlockSt.iBlockStatements.entrySet())
					{
						...
						if (iSt.type == StatType.BEGINBLOCK || iSt.type == StatType.LOOPBLOCK)
						{
							if (iBlkSt.iBlockStatements.size() > 0)
							{
								updatedVal =  processBlkSt(iBlkSt, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
								statementCnt.add(statementCnt.size() + 1);
							}
						}
...
//									manualChanges.add("dsError_" + origLine+";"+key);
						...
						{
							tmp = "";
							if (iCallEx.type == StatType.CALLSTAT)
						...
						if (iAssig != null)
						{
							...
							{
								tmp = "";
								tmp += getAssignmentVal (iAssig, updatedVal, newDbId, currentDbId) + "\n";
						...
						if (iIf != null)
						{
							if (iSt.type == StatType.IFBLOCK ||  iSt.type == StatType.ELSEIFBLOCK)
							{
								tmp = "";
								tmp += getIfBlockVal (iIf, tmp, newDbId, currentDbId, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList) + "\n";
						...
						if (iFor != null)
						{
							...
							{
								tmp = "";
								tmp += getForBlockVal (iFor, tmp, newDbId, currentDbId, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList) + "\n";
								updatedVal += tmp;
								iFor=null;
								statementCnt.add(statementCnt.size() + 1);
							}
						...
						if (iQuery != null)
						{
							if (iSt.type == StatType.SQLQUERY)
							{
								...
								{
									tmp = "";
									tmp += "\n"+ tmpTab + "ds_v_sqlCommand = '" + getSQLDet(iSt.originalLineNo, iQuery.sqlQuery, currentDbId, newDbId, qryCnt, rptLogInfo, configHash, tmp, tabCnt, sqLValList, totalAttributeCount, totalTransCount, manualChangesList, sqlQryList).trim()+"';\n";
									tmp += "\n"+ tmpTab + "snowflake.execute (ds_v_sqlCommand);\n";
									if (sqlQryList == null)
...
//									statementCnt.add(statementCnt.size() + 1);
						...
						if (iWhile != null)
						{
							...
							{
								tmp = "";
								tmp += getWhileBlockVal (iWhile, tmp, newDbId, currentDbId, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList) + "\n";
						...
						if (iDoWhile != null)
						{
							...
							{
								tmp = "";
								tmp += getDoBlockVal (iDoWhile, tmp, newDbId, currentDbId, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList) + "\n";
	...
	//						if (iSt.type == StatType.IFBLOCK)
							{
								tmp = "";
								tmp += getThrowsVal (iThrows, tmp, newDbId, currentDbId, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList) + "\n";
								throwsVal.add(tmp);
//								updatedVal += tmp;
								iThrows=null;
								statementCnt.add(statementCnt.size() + 1);
...
//								updatedVal += tmpTab + "RAISE;\n";
						...
						if (iExcep != null)
						{
							...
							{
								tmp = "";
								tmp += getExceptionBlockVal (iExcep, tmp, newDbId, currentDbId, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList) + "\n";
		...
		private static String getCursorValues(dsCursor iCursor, String updatedVal, int newDbId, int currentDbId, ReportLogInfo rptLogInfo, LinkedHashMap<String, LinkedHashMap<String, String>> configHash, int qryCnt, int tabCnt, ArrayList<String> throwsVal, ArrayList<Integer> statementCnt,
				LinkedHashMap<ArrayList<String>, Integer> manualChangesList, LinkedList<SQLValue> sqLValList, ArrayList<String> totalAttributeCount, ArrayList<String> totalTransCount, LinkedList<iStatement> sqlQryList) 
		{
//			updatedVal = iCursor.cursorName;
			String tmpTab ="";
			if (tabCnt > 0 )
				tmpTab = getTabSpace (tabCnt, tmpTab);
			if (iCursor.sqlQuery != null)
			{
				...
				tmp += "ds_v_sqlCommand = '" + getSQLDet (orgLineNo,iCursor.sqlQuery, currentDbId, newDbId, qryCnt, rptLogInfo, configHash, tmp, tabCnt, sqLValList, totalAttributeCount, totalTransCount, manualChangesList, sqlQryList).replace("\n","").trim()+"';" + "\n";
				updatedVal = tmp +"\n"+ tmpTab + "snowflake.execute (ds_v_sqlCommand);\n";
			}
...
//					updatedVal += tmpTab + "WHEN ";
					...
					{
							updatedVal = processBlkSt(iBlk, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
					}
					else
					{
						iStatement iSt = null;
...
//				updatedVal += tmpTab + "THEN \n";
				...
				{
						tabCnt++;	
						updatedVal = processBlkSt(iExcep.thenBlock, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
						updatedVal += "}" ;
				}
			}
			return updatedVal;
		...
		private static String getSQLDet(int originalLineNo, SQLQuery sqlQuery, int currentDbId, int newDbId, int qryCnt, ReportLogInfo rptLogInfo, LinkedHashMap<String, LinkedHashMap<String, String>> configHash, String tmp, int tabCnt, LinkedList<SQLValue> sqLValList, ArrayList<String> totalAttributeCount, ArrayList<String> totalTransCount, LinkedHashMap<ArrayList<String>, Integer> manualChangesList, LinkedList<iStatement> sqlQryList) 
		{
			...
			{
				tmp = "\n "
						+ "\n " + tmpTab + "var sql_command = " + "\"" + sqlStmts.replace("\n", " ").replace("  ", " ") + "\""
						+ "\n " + tmpTab + "try"
						+ "\n " + tmpTab + "{" + "" 
						+ "\n " + tmpTab + "\tsnowflake.execute ( {sqlText: sql_command});" 
						+ "\n " + tmpTab + "}"
						+ "\n " + tmpTab + "catch (err)"
						+ "\n " + tmpTab + "{"
						+ "\n " + tmpTab + "\treturn \"Failed : \" + err;"
						+ "\n " + tmpTab + "}  "
						+ "\n";
				qryCnt++;
			}
			...
			if (iCallEx.procName != null)
			{
				...
				for (int i = 0; i < tkn.size(); i++) 
				{
					if (tkn.get(i).type.name().equalsIgnoreCase("varwithdot"))
					{
						...
						if (callProcName.equalsIgnoreCase("sysExecSQL") && newDbId == ToolTypeConstants.SNOWFLAKE)
						{
		...
		private static String getExecDetail(String callProcName, int newDbId, String tmpTab, ArrayList<SQLExpr> argsList, int currentDbId) 
		{
			String  tmpName = "";
			
			String args = "";
			if (argsList.size() > 0)
				args = getArgValue (argsList, newDbId, currentDbId);
				
//			tmpName += tmpTab + "\n" + tmpTab + "try \n";
//			tmpName += tmpTab + "{\n";
			tmpName += "\n";
			tmpName += tmpTab + "snowflake.execute ("+args+");\n";
//			tmpName += tmpTab + "}\n ";
//			tmpName += tmpTab + "catch(err) \n";
//			tmpName += tmpTab + "{ \n";
//			tmpName += tmpTab + "\treturn 'Failed : ' + err;\n";
//			tmpName += tmpTab + "}\n";
...
//				tabCnt++;
				statementCnt.add(statementCnt.size() + 1);
				tmp = processBlkSt(iBlk, newDbId, currentDbId, tmp, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
//				tabCnt--;
			}
			/*if (newDbId == ToolTypeConstants.SNOWFLAKE)
			{
				tmp += tmpTab + "}\n ";
				tmp += tmpTab + "catch(err) \n";
				tmp += tmpTab + "{ \n";
				tmp += tmpTab + "\treturn 'Failed : ' + err;\n";
				tmp += tmpTab + "}\n";
			}*/
			...
			if (iFor.condition != null)
			{
				...
				if (iCond != null)
				{
					...
					if (iCond.sqlExpr != null)
					{
						...
						{
							//tempVal += iCond.sqlExpr.value.substring(0, iCond.sqlExpr.value.indexOf("(")) +"(";
							...
							for (int i = 0; i < sqlExp.args.size(); i++) 
							{
								...
								{
									String tmp = "";
									tmp += "\n"+tmpTab + "var cur_SQL_command = '" + getSQLDet (orgLineNo,sqlQry, currentDbId, newDbId, qryCnt, rptLogInfo, configHash, tmp, tabCnt, sqLValList, totalAttributeCount, totalTransCount, manualChangesList, sqlQryList).replace("\n","").trim()+"';" + "\n";
									tmp += tmpTab + "var stmt = snowflake.createStatement ({sqlText: cur_SQL_command}); \n";
									tmp += tmpTab + "var "+condVal+" = snowflake.execute (stmt); \n";
									tempVal += tmp+"\n";//sqlQryVal.sqlValue;
								}
...
//					}
					tabCnt++;
					tempVal +=  processBlkSt(iFor.forBlock, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
				}
			}
//			if (!(newDbId == ToolTypeConstants.SNOWFLAKE))
//				tempVal += tmpTab + "END FOR;\n";
		...
		private static String getDoBlockVal(dsDoWhile iDoWhile, String updatedVal, int newDbId, int currentDbId, ReportLogInfo rptLogInfo, LinkedHashMap<String, LinkedHashMap<String, String>> configHash, int qryCnt, int tabCnt, ArrayList<String> throwsVal, ArrayList<Integer> statementCnt, LinkedHashMap<ArrayList<String>, Integer> manualChangesList, LinkedList<SQLValue> sqLValList, ArrayList<String> totalAttributeCount, ArrayList<String> totalTransCount, LinkedList<iStatement> sqlQryList) 
		{
			...
			if (iDoWhile.doBlock != null)
			{
				if (iDoWhile.doBlock.iBlockStatements.size() > 0)
				{
					tabCnt++;
					updatedVal += tmpTab + "{\n";//DO \n";							
					updatedVal =  processBlkSt(iDoWhile.doBlock, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
				}
			}
//			if (!(newDbId == ToolTypeConstants.SNOWFLAKE))
				updatedVal += tmpTab + "}\n";// END WHILE;\n";
		...
		private static String getWhileBlockVal(dsWhile iWhile, String updatedVal, int newDbId, int currentDbId, ReportLogInfo rptLogInfo, LinkedHashMap<String, LinkedHashMap<String, String>> configHash, int qryCnt, int tabCnt, ArrayList<String> throwsVal, ArrayList<Integer> statementCnt, LinkedHashMap<ArrayList<String>, Integer> manualChangesList, LinkedList<SQLValue> sqLValList, ArrayList<String> totalAttributeCount, ArrayList<String> totalTransCount, LinkedList<iStatement> sqlQryList) 
		{
			...
			if (iWhile.whileBlock != null)
			{
				...
				{
					updatedVal =  processBlkSt(iWhile.whileBlock, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
				}
			}
//			if (!(newDbId == ToolTypeConstants.SNOWFLAKE))
				updatedVal += "}";// END WHILE;\n";
			return updatedVal;
		}
		
	
		private static String processElseIf(iBlockStatement elseIfBlock, int currentDbId, int newDbId, ReportLogInfo rptLogInfo, LinkedHashMap<String, LinkedHashMap<String, String>> configHash, int qryCnt, int tabCnt, ArrayList<String> throwsVal, ArrayList<Integer> statementCnt, LinkedHashMap<ArrayList<String>, Integer> manualChangesList, LinkedList<SQLValue> sqLValList, ArrayList<String> totalAttributeCount, ArrayList<String> totalTransCount, LinkedList<iStatement> sqlQryList, String tmpTab) 
		{
			String updatedVal = "";
			tmpTab = "";
			if (tabCnt > 0)
			...
			if (elseIfBlock.iBlockStatements.size() > 0)
			{
				for(Entry<Float, iStatement> entrySet : elseIfBlock.iBlockStatements.entrySet())
				{
					...
					if (iSt != null)
					{
						...
						{
//							updatedVal += "\n" +tmpTab +"THEN \n";
							updatedVal = processBlkSt((iBlockStatement) entrySet.getValue(), newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
//							updatedVal += tmpTab ;
						}
		...
		{
//			System.out.println("b  "+statementCnt+""+updatedVal);
			String tmpTab = "";
			if (tabCnt > 0)
				tmpTab = getTabSpace (tabCnt, tmpTab);
			...
			if (iIf.elseIfBlock != null)
			{
				...
				{
					String tempVal = "";
					tempVal  = processElseIf (iIf.elseIfBlock, currentDbId, newDbId, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList, tmpTab);
			...
			if (iIf.thenBlock != null)
			{
				if (iIf.thenBlock.iBlockStatements.size() > 0)
				{
						updatedVal += tmpTab + "{\n";//THEN \n";
						tabCnt++;	
						updatedVal = processBlkSt(iIf.thenBlock, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList, totalAttributeCount, totalTransCount, sqlQryList);
						updatedVal += tmpTab + "}\n";
				}
			}
			if (iIf.elseBlock != null)
			{
				if (iIf.elseBlock.iBlockStatements.size() > 0)
				{
					...
					updatedVal += tmpTab +"{ \n";
					updatedVal = processBlkSt(iIf.elseBlock, newDbId, currentDbId, updatedVal, rptLogInfo, configHash, qryCnt, tabCnt, throwsVal, statementCnt, manualChangesList, sqLValList,totalAttributeCount, totalTransCount, sqlQryList);
					updatedVal += tmpTab + "}\n";
				}
			}
//			if (!(newDbId == ToolTypeConstants.SNOWFLAKE))
//				updatedVal +=  tmpTab +"}\n";// END IF;\n";	
//			System.out.println("a  "+statementCnt+""+updatedVal);
			return updatedVal;
		}
